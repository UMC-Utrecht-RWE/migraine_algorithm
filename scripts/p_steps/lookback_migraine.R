...
T5_result<-lookback_test(expos_data=Mig_T5, preg_data=my_PREG, lookback=my_lookback)

T6_result<-lookback_test(expos_data=Mig_T6, preg_data=my_PREG, lookback=my_lookback)
...
for (j in 1: length(A_group)){
  fwrite(A_group[j], paste0(output_alg_A, "/", "LB_",(-1*my_lookback), "_Mig_A",j,".csv"))
}

for (j in 1: length(T_group)){
  fwrite(T_group[j], paste0(output_alg_T, "/","LB_",(-1*my_lookback), "_Mig_T",j,".csv"))
}

for (j in 1: length(S_group)){
  fwrite(S_group[j], paste0(output_alg_S, "/","LB_",(-1*my_lookback),"_Mig_S", j,".csv"))
}
